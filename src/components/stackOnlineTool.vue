...
<template>
  <div>
    ...
    <h3 class="stack">Online Tools:</h3>
    <ul class="list stack">
      ...
      <li v-for="item in stackOnlineTools" class="list__item stack" :key="item._id">
        <img
          class="projects__img"
          v-if="item.techLogo"
...
const query = `*[_type == "stack" && stackType == "onlineTools"] {
  stackType,
  techName,
  techLogo,
}[0...50]`;
  ...
  methods: {
    ...
    fetchData() {
      ...
      sanity.fetch(query).then(
        stackOnlineTools => {
          this.loading = false;
          this.stackOnlineTools = stackOnlineTools;
        },
        error => {
          this.error = error;
        }

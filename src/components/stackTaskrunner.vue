...
<template>
  <div>
    ...
    <h3 class="stack">Taskrunners & Bundlers:</h3>
    <ul class="list stack">
      ...
      <li v-for="item in stackTaskrunnerBundler" class="list__item stack" :key="item._id">
        <img
          class="projects__img"
          v-if="item.techLogo"
...
const query = `*[_type == "stack" && stackType == "taskrunnersBundlers"] {
  stackType,
  techName,
  techLogo,
}[0...50]`;
...
export default {
  ...
  name: "TechStackTaskrunnersBundler",
  data() {
    return {
      loading: true,
      stackTaskrunnerBundler: []
    };
  },
  created() {
    this.fetchData();
  ...
  methods: {
    ...
    fetchData() {
      ...
      sanity.fetch(query).then(
        stackTaskrunnerBundler => {
          this.loading = false;
          this.stackTaskrunnerBundler = stackTaskrunnerBundler;
        },
        error => {
          this.error = error;
        }
